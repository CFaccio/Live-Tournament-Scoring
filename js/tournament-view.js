...
window.showRatingEdit = (uid, currentRating) => {
  document.getElementById(`rating-display-${uid}`).style.display = 'none';
  document.getElementById(`rating-input-${uid}`).style.display = 'flex';
  const input = document.getElementById(`rating-val-${uid}`);
  if (input) { input.focus(); input.select(); }
};

window.cancelRatingEdit = (uid) => {
  document.getElementById(`rating-display-${uid}`).style.display = 'flex';
  document.getElementById(`rating-input-${uid}`).style.display = 'none';
};

window.saveRatingInline = async (uid) => {
  const input = document.getElementById(`rating-val-${uid}`);
  const rating = parseFloat(input?.value);
  if (isNaN(rating) || rating < 0 || rating > 10) {
    showToast('Please enter a valid rating between 0 and 10.', 'error');
    return;
  }
  const t = currentTournament;
...
window.removePlayerFromTournament = async (uid) => {
  const t = currentTournament;
  if (!t || t.organiserId !== currentUser.uid || t.phase !== 'waiting') return;
  const players = (t.players || []).filter(p => p.uid !== uid);
  await updateDoc(doc(db, 'tournaments', t.id), { players });
